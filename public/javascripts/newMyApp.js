/**
 * Created by cailong on 2017/1/17.
 */
...
app.controller('itunesSearchControl', function($scope, $http,$location, FileUploader){
    ...
    $scope.inviteUrl = "http://qicai.leanapp.cn/user/register/" + getCookie("userIdCookie");
    ...
    function releaseTask(){
        var excCount = $scope.displayDemandTemplate.amountPerDay;
        var excPrice = $scope.displayDemandTemplate.excUnitPrice;
        if ($scope.displayDemandTemplate.sendType == undefined){
            $scope.displayDemandTemplate.sendType = 'now';
        }
    ...
    // 查看ASO关键词
    $scope.checkTheKeywords = function(appId){
        window.open('https://aso100.com/app/keyword/appid/' + appId + '/' + 'country/cn');
        //window.open('http://u.qimai.net/1be');
    };
    ...
    function moneyCheck(){
        ...
        if(excCount != undefined && excCount > 0){
            $scope.excUnitPrice = $scope.displayDemandTemplate.excUnitPrice;
            var taskMoney = excCount * $scope.excUnitPrice;
            if(taskMoney > $scope.usermoney){
                $scope.insufficientFund = true;
                console.log('money ' + $scope.usermoney + 'not enough, need ' + $scope.excUnitPrice + ', total' + taskMoney);
            }
            else{
                $scope.insufficientFund = false;
                console.log('money ' + $scope.usermoney + 'not enough, need ' + $scope.excUnitPrice + ', total' + taskMoney);
    ...
    $scope.amountPerDayCount = function(){
        ...
        if ($scope.displayDemandTemplate.taskType == '评论' || $scope.displayDemandTemplate.taskType == '定制评论'  ){
            $scope.displayDemandTemplate.ExcUnitPrice = $scope.excUnitPrice * $scope.displayDemandTemplate.amountPerDay;
        }else {
            $scope.displayDemandTemplate.ExcUnitPrice = $scope.excUnitPrice * $scope.displayDemandTemplate.amountPerDay;
        }
    };
    ...
    putPeriodDate = function(){
        $scope.displayDemandTemplate.putNumberOfDays = document.getElementById('putCount').value;;
        $scope.calYCoin();

        console.log('--------' + $scope.displayDemandTemplate.putNumberOfDays);
    };
    ...
    $scope.createAsoPlan = function () {
        ...
        $scope.displayDemandTemplate.appleId = $scope.selectedApp.appleId;
        $scope.displayDemandTemplate.score = $scope.displayDemandTemplate.Score;
        if ($scope.usermoney < $scope.displayDemandTemplate.excUnitPrice){
            toastr.error('Y币不足发布本次任务');
        }
        else if ($scope.displayDemandTemplate.amountPerDay <=0 ){
            toastr.error('任务数量不对');
        }
        else if($scope.displayDemandTemplate.taskType == undefined){
            toastr.error('请选择任务类型');
        }else if($scope.displayDemandTemplate.ranKing == undefined || $scope.displayDemandTemplate.ranKing > 200){
            toastr.error('排名超过了' + 200 + '名');
        }
        else {
            ...
            if($scope.displayDemandTemplate.taskType == '定制评论') {
                if($scope.displayDemandTemplate.reviewHeaderOptions == undefined || $scope.displayDemandTemplate.reviewHeaderOptions == ''){
                    ...
                    toastr.error('未上传评论模板');
                    return;
                }
            }

            if($scope.displayDemandTemplate.taskType == '评论') {
                if($scope.displayDemandTemplate.titleKeyword == undefined || $scope.displayDemandTemplate.titleKeyword == ''){
                    $scope.createAsoPlanLoading = false;
                    toastr.error('评论标题关键词未填写');
                    return;
                }
                if($scope.displayDemandTemplate.commentKeyword == undefined || $scope.displayDemandTemplate.commentKeyword == ''){
                    $scope.createAsoPlanLoading = false;
            ...
            $http.post(createAsoPlanURL, {'createAsoPlanInfo': $scope.displayDemandTemplate})
                .success(function(response){
                    if (response.errorId == 0){
                        $scope.errorId = response.errorId;
                        $('#releaseTaskApp1').modal('hide');
                        //toastr.success('发布成功', {timeOut: 2000});
                        $('#taskReleaseSuccess').modal('show');

                        $scope.taskSuccessSub = function(){
                            location.reload();
                        }
                    }else {
                        toastr.error(response.errorMsg, {timeOut: 3000});
                    }
                })
                .finally(function () {
    ...
    $scope.saveAction = function(){
        var saveTaskURL = 'myapp/saveTask';
        $http.post(saveTaskURL,  {'saveTaskObject': $scope.displayDemandTemplate}).success(function(response){
            if (response.errorId == 0){
                location.reload();
            }
        })
