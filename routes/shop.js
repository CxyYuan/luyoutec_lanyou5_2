    ...
    function modifyPlanObject(createPlanObject, responseMessage) {
        if(createAsoPlanInfo.actionId != undefined){
            ...
            if(createAsoPlanInfo.taskType == '评论'){
                createPlanObject.set('comment4G', createAsoPlanInfo.comment4G);
                createPlanObject.set('commentKeys', createAsoPlanInfo.commentKeys);
                createPlanObject.set('commentContentKeys', createAsoPlanInfo.commentContentKeys);
            }
            ...
            if(createAsoPlanInfo.sendType == 'now'){
                //立马发布任务，计划结束
                createPlanObject.set('planDeliverStatus', 'planEnd');
            }

            //2016双11 5折优惠
            var planObject = createPlanObject;
            //6折计划也未生效过
            if( canHaveDiscount == true && planObject.get('lastReleaseTask') == undefined &&
                planObject.get('sendType') == 'timer' && planObject.get('taskType') == '下载' && planObject.get('formalCheck') == 'formalCheck' &&
                planObject.get('taskLastDay') >= 3 && planObject.get('taskCountPerDay') >= 30 && planObject.get('taskCountPerDay') <= 50 &&
                planObject.get('startRanking') >= 100 && planObject.get('startRanking') <= 200
            ){
                createPlanObject.set('planDisCount', 6);
                createPlanObject.set('discountDate', '20161111');
            }else {
                //普通8折优惠
                if(createPlanObject.get('planDisCount') == 6){
                    //可能因为修改计划取消了5折投放
                    createPlanObject.unset('discountDate');
                }

                createPlanObject.set('planDisCount', 10);
            }
        }
        ...
        createPlanObject.save().then(function(date){
            ...
            if(createAsoPlanInfo.sendType == 'now' && createAsoPlanInfo.actionId == undefined){
                ...
                planQuery.first().then(function(tempPlanObject) {
                    ...
                    //send Task now
                    var ret = tryPriceUtil.planToReleaseTaskObject(tempPlanObject, parseInt(createAsoPlanInfo.ranKing), needToBatchSaveObject);
                    if(ret == false){
