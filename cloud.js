 ...
 * 定时器重构：秒赚吧定时器分成2个 —— 处理领取任务未完成 && 处理提交任务超时未处理
 * 定时器重构：秒赚吧 小马定时器分开来处理 —— 解锁2个模块独立的任务逻辑
        ...
        for (var i = 0; i < planObjects.length; i++) {
            ...
            if(isNeedSendTask == true || testPlanObjectIds.length > 0){
                ...
                (function (tempPlanObject) {
                    ...
                    var maxASOKey = tempPlanObject.get('startRanking');
                    ...
                    //投放新的计划
                    tryPriceUtil.aso100RankUrl(tempPlanObject.get('asoKey'), 1, tempPlanObject.get('appleId'), maxASOKey,
...
AV.Cloud.define('recordUserASOKeyRankTimer', function(request, response){
    ...
    query.find().then(function(ASOKeyRanks){
        for (var i = 0; i < ASOKeyRanks.length; i++){
            ...
            var maxASOKey = ASOKeyRankObject.get('rank');
            ...
            (function (asoKey, appleId, asoObject) {
                tryPriceUtil.aso100RankUrl(asoKey, 1, appleId, 1, maxASOKey,
                    function (asoKeyRank, errorId, errorMsg) {
                        if(errorId == 0){
